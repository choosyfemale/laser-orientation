...
-   It's got a ton of variables regarding student performance and demographics.

-   Course ID is a composite variable.
...
-   Variables to describe each student.
...
```{r}
d %>% # old pipe to the rescue!!!
  select(student_id, total_points_possible, total_points_earned, uv)
```
...
-   Now the cut off is greater than 90, so all the A folks!
...
```{r}
ggplot(d, aes(x = FinalGradeCEMS)) +
  geom_histogram(fill = "darkgoldenrod")
```
...
```{r}
ggplot(d, aes(x = TimeSpent_hours, y = FinalGradeCEMS)) +
  geom_point()
```
